...
export default function StatsBar() {
  ...
  if (!stats) return <div className="h-16 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 animate-pulse" />
  ...
  const cards = [
    ...
    { label: 'Active Sources', value: stats.active_sources.toString(), icon: Layers, color: 'text-indigo-600 dark:text-indigo-400' },
    { label: 'HTTP Sent', value: (stats.per_transport.http || 0).toLocaleString(), icon: null, color: 'text-purple-600 dark:text-purple-400' },
    { label: 'Syslog Sent', value: (stats.per_transport.syslog || 0).toLocaleString(), icon: null, color: 'text-yellow-600 dark:text-yellow-400' },
    { label: 'WEC Sent', value: (stats.per_transport.wec || 0).toLocaleString(), icon: null, color: 'text-cyan-600 dark:text-cyan-400' },
  ]
