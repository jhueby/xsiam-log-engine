...
### Scenario Runner
`ScenarioRunner` (`engine/scenarios/runner.py`) fires timed, entity-correlated event sequences across sources, independent of each source's own steady-state EPS loop. A run is a background `asyncio.Task` per scenario, stepping through `delay`/`jitter`-timed calls to `Engine.fire_scenario_event()` — the same send-and-record path normal traffic uses, so scenario events appear in stats/log-ring/dashboards identically to anything else. Scenario definitions are `.yaml`/`.yml` files under `engine/scenarios/definitions/`, read by `scenarios/loader.py` at startup and again on `POST /api/control/reload`.

The loader is the single validation point: it normalizes every step (`source`, `delay`, `jitter`, `overrides`) and drops any definition that can't satisfy those assumptions, because nothing downstream re-checks — the API's `_scenario_to_info` and `ScenarioRunner.start()` both index step fields directly, so a malformed step that got through would 500 the scenarios list and hide *every* scenario. For the same reason parse errors are caught per file: `load_scenarios()` runs from `ScenarioRunner.__init__` → `Engine.__init__`, so an uncaught `YAMLError` there stops the whole engine from starting, not just scenarios. Reloading swaps the definitions dict only; in-flight runs hold their own resolved copy of the steps and are unaffected.
